...
const keyboard4 = [['Edit bot ✏️', 'Toggle strategy 🔄'], ['Add bot ✚'], ['Back ◀']];
const keyboard3 = [['Start 🚀', 'Stop ⛔'], ['Status ✅', 'Statistics 📈'], ['Settings ⚙']];
const keyboard2 = [
    [
        {
            text: 'Edit name ✏',
...
function generateEditKeyboard(data, id) {
    ...
    const keyboard = [
        [
            {
                text: 'Edit long bot',
                callback_data: `edit_long: ${data.bots[0].type === 'long' ? id + '|0' : id + '|1'}`,
            },
            {
                text: 'Edit short bot',
                callback_data: `edit_short:${data.bots[1].type === 'short' ? id + '|1' : id + '|0'}`,
            },
        ],
    ];
...
bot.on('message', async (msg) => {
    ...
    if (msg.reply_to_message) {
        ...
        if (msg.reply_to_message.text === 'Please forward the bot name to this message') {
        ...
        if (msg.reply_to_message.text === 'Please forward the API_KEY to this message') {
        ...
        if (msg.reply_to_message.text === 'Please forward the API_SECRET to this message') {
        ...
        await Promise.all(
            loadData.map(async (element) => {
                ...
                if (status.enable && status.deal.length) {
                    if (!isEnabled) {
                        bot.sendMessage(chatId, 'There are running bots with open deals, close open deals?', {
                            reply_markup: {
                                inline_keyboard: [
                                    [
    ...
    if (msg.text === 'Status ✅') {
        ...
        await Promise.all(
            loadData.map(async (element) => {
                ...
                const msg = `${
                    res.enable ? bot_name + ' launched ✅ | ' + type : bot_name + ' is not running ❌ | ' + type
                } \nActive deal: ${res.deal.length ? '\nProfit: ' + res.deal[0].usd_final_profit + ' USD' : 'none'}`;
                message += msg + '\n-----\n';
            }),
        );
        bot.sendMessage(chatId, message);
    ...
    if (msg.text === 'Statistics 📈') {
        ...
        await Promise.all(
            loadData.map(async (element) => {
                ...
                const api_secret = encrypt.dencrypt(element.secret);
                const tc = new threeCommas(api_key, api_secret);
                const stat = await tc.statsBot(element.long, element.short, api_key, api_secret);
                message +=
                    element.name +
                    '\nTotal profit: ' +
    ...
    if (msg.text === 'Toggle strategy 🔄') {
        ...
        bot.sendMessage(chatId, newType === 'short' ? 'You choosed short strategy' : 'You choosed long strategy');
        return;
    }
    if (msg.text === 'Add bot ✚') {
